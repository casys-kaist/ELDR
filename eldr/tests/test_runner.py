...
class RunnerTests(unittest.TestCase):
    ...
    def test_preflight_finds_rocm_smi_outside_path_and_keeps_gpu_guards(self):
        for usage, rdma, bus, error in (
            (0, "4: ACTIVE", "0001:01:00.0", None),
            (1, "4: ACTIVE", "0001:01:00.0", "GPU already in use"),
            (0, "2: INIT", "0001:01:00.0", "RDMA rail is not active"),
            (0, "4: ACTIVE", "0001:01:00.8", "Invalid GPU PCI bus"),
            (0, "4: ACTIVE", "1:1:0.0", "Invalid GPU PCI bus"),
            (0, "4: ACTIVE", "xxxx:01:00.0", "Invalid GPU PCI bus"),
        ):
            with self.subTest(usage=usage, rdma=rdma, bus=bus):
                ...
                def remote(host, argv, *, usage=usage, rdma=rdma, bus=bus, **kwargs):
                    output = ""
                    if argv[:2] == ["sh", "-c"]:
