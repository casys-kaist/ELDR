...
# Modified for ELDR: expert-locality-aware serving integration.
...
class NixlConnectorScheduler:
        ...
        with zmq_ctx(zmq.ROUTER, path) as sock:
            ...
            while not stop_event.is_set():
                try:
                    ...
                    continue
                frames = [frame.bytes for frame in received]
                # ZMQ_PROBE_ROUTER sends an empty body on connection. It is
                # not a metadata request and must not terminate the listener.
                if len(frames) == 2 and frames[1] == b"":
                    continue
                try:
                    # REQ adds an empty delimiter; REQ_CORRELATE additionally
                    # adds a four-byte request ID. Preserve that reply envelope.
                    normal = len(frames) == 3 and frames[1] == b""
                    correlated = (
                        len(frames) == 4 and len(frames[1]) == 4 and frames[2] == b""
                    )
                    if not (normal or correlated):
                        raise ValueError("invalid request envelope")
                    query = msgspec.msgpack.decode(frames[-1])
                    if (
                        not isinstance(query, (list, tuple))
                        or len(query) != 2
                        or query[0] != GET_META_MSG
                    ):
                        raise ValueError("invalid metadata request")
                    target_tp_rank = query[1]
                    if (
                        type(target_tp_rank) is not int
                        or target_tp_rank not in encoded_data
                    ):
                        raise ValueError("unavailable metadata rank")
                except (ValueError, msgspec.DecodeError):
                    # Never expose wire payloads or let a malformed peer kill
                    # the shared listener. Log frame sizes and peer address at
                    # powers of two so repeated bad traffic cannot flood logs.
                    rejected += 1
                    if rejected & (rejected - 1) == 0:
                        peer = "unknown"
                        if received:
                            try:
                                peer = received[0].get("Peer-Address")
                            except zmq.ZMQError:
                                peer = "unavailable"
                        logger.warning(
                            "Rejected NIXL handshake: count=%d peer=%s frame_sizes=%s",
                            rejected,
                            peer,
                ...
                sock.send_multipart((*frames[:-1], encoded_data[target_tp_rank]))
    ...
    def request_finished(
        ...
        # Piggyback the completed prefill signature on the existing KV handoff.
        if is_p_node and params.get("prefill_first"):
            assert len(request.output_token_ids) == 1
            kvt.update(
                prefill_first_token_id=request.output_token_ids[0],
                prefill_prompt_tokens=request.num_prompt_tokens,
                prefill_continues=params.get("prefill_continues", False),
            )
        if is_p_node and delay_free_blocks and os.environ.get("ELDR") == "1":
            signature = request.eldr_signature
            if signature is None:
                raise RuntimeError("Completed prefill has no ELDR signature")
            request.eldr_signature = None
            kvt["eldr_sig"] = base64.b64encode(signature).decode()
        return delay_free_blocks, kvt
