    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        for layer_name, cache_or_caches in xfer_buffers.items():
            ...
            for cache in cache_list:
                ...
                if not self.use_mla and tensor_size_bytes != curr_tensor_size_bytes:
                    # Heterogeneous per-layer KV sizes (e.g. gemma-4: local
                    # sliding-window layers head_dim=256 vs global layers 512).
                    # Per-layer size is recorded in caches_data and FA descriptors
                    # are built per-layer (block_len_per_layer[i] in _build_fa_*),
                    # so 1:1 TP (no head-split) transfers each layer correctly.
                    # Head-split hetero (TP>1) is rejected per-layer at handshake.
                    logger.warning_once(
                        "NIXL: heterogeneous per-layer KV cache sizes detected "
                        "(%s); using per-layer descriptors (1:1 TP only).",
                        layer_name,
                    )
    ...
    def _validate_remote_agent_handshake(
        ...
        if self.use_mla or self.transfer_topo.is_kv_replicated(remote_engine_id):
            ...
            and (len(set(nixl_agent_meta.block_lens)) > 1
                 or len(set(self.block_len_per_layer)) > 1)
        ):
            # Heterogeneous per-layer block lengths (e.g. gemma-4: local
            # sliding-window head_dim=256 vs global 512). With 1:1 TP and no
            # block-size mismatch there is no head-split, so each layer transfers
            # its full per-layer block as-is; _build_fa_local/_build_fa_remote
            # already stride per-layer (block_len_per_layer[i] / block_lens[i]).
            # Validate per-layer instead of requiring one uniform block length.
            assert len(nixl_agent_meta.block_lens) == len(self.block_len_per_layer), (
                "P and D must expose the same number of KV layers"
            )
            for i in range(len(self.block_len_per_layer)):
                assert self.block_len_per_layer[i] == nixl_agent_meta.block_lens[i], (
                    "P/D per-layer KV block sizes must match for heterogeneous "
                    f"(per-layer head_dim) models; layer {i}: "
