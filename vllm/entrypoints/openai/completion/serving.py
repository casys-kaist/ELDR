...
class OpenAIServingCompletion(OpenAIServing):
    ...
    async def completion_stream_generator(
        ...
        # The proxy already delivered this generated token, not a prompt token.
        # The decoder's detokenizer suppresses its text; usage still counts it.
        seeded = (request.kv_transfer_params or {}).get("prefill_first_token_id")
        if seeded is not None:
            previous_num_tokens = [1] * num_choices * num_prompts
            created_time = request.kv_transfer_params["prefill_created"]
        has_echoed = [False] * num_choices * num_prompts
        if seeded is not None and request.return_token_ids:
            has_echoed = [True] * num_choices * num_prompts
