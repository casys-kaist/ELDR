...
class RequestState:
    ...
    def from_new_request(
        ...
        if sampling_params and detokenizer is not None:
            from vllm.eldr.prefill_first import prime_detokenizer

            state.sent_tokens_offset = prime_detokenizer(detokenizer, sampling_params)
        return state
    ...
    def process_outputs(
        ...
        for engine_core_output in engine_core_outputs:
            ...
            if request_output := req_state.make_request_output(
                ...
                if (
                    kv_transfer_params
                    and "prefill_first_token_id" in kv_transfer_params
                ):
                    # A producer stopped only for handoff is not a user-visible
                    # termination: do not flush an incomplete stop-string prefix.
                    continues = (
                        kv_transfer_params["prefill_continues"]
                        and finish_reason != FinishReason.STOP
                    )
                    kv_transfer_params["prefill_continues"] = continues
                    if continues:
