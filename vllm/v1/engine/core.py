...
class EngineCore:
    ...
    def eldr_capture(self, mode: str):
        """Serialized benchmark-only phase transition; never preempts requests."""
        from vllm.eldr.control import capture_with_empty_prefix

        if not os.environ.get("ELDR_CONTROL_TOKEN"):
            raise ValueError("ELDR capture control was not enabled at startup")
        return capture_with_empty_prefix(self, mode)
...
class EngineCoreProc(EngineCore):
    ...
    def _process_engine_step(self) -> bool:
        ...
        # Yield while waiting for KV transfers so handshake/ACK threads progress.
