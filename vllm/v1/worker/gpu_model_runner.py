...
from vllm.eldr import signature as eldr_sig  # ELDR (no-op unless env ELDR=1)
from vllm.eldr import transport as eldr_transport
...
class AsyncGPUModelRunnerOutput(AsyncModelRunnerOutput):
    def __init__(
        ...
        vocab_size: int,
        eldr_signatures: eldr_transport.PendingSignatureCopy | None = None,
    ):
        ...
        self._eldr_signatures = eldr_signatures
        ...
        with torch.cuda.stream(async_output_copy_stream):
            ...
            if self._eldr_signatures is not None:
                self._eldr_signatures.enqueue_copy()
    ...
    def get_output(self) -> ModelRunnerOutput:
        ...
        if self._eldr_signatures is not None:
            output.eldr_signatures = self._eldr_signatures.finish_after_sync()
            self._eldr_signatures = None
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        eldr_transport.validate_config(vllm_config)
        if os.environ.get("ELDR_SCRATCH_BUCKETS") == "1":
            from vllm.eldr.scratch import install
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            initial_output_ids = first_token_ids(sampling_params)
            ...
            if (
                ...
                if initial_output_ids:
                    advance_seeded_generator(
                        generator, self.input_batch.vocab_size, self.device
                    )
            else:
                generator = None
            ...
            req_state = CachedRequestState(
                ...
                output_token_ids=initial_output_ids,
    ...
    def sample_tokens(
        ...
        pending_eldr_signatures = None
        if eldr_transport.ENABLED:
            pending_eldr_signatures = eldr_transport.prepare(
                scheduler_output,
                self.input_batch,
                self.vllm_config,
                seq_lens=self.seq_lens,
                discard_mask=self.discard_request_mask.gpu,
            )
        ...
        if not self.use_async_scheduling:
            if pending_eldr_signatures is not None:
                output.eldr_signatures = pending_eldr_signatures.finish_synchronously()
            return output
        ...
        with record_function_or_nullcontext(
            ...
            async_output = AsyncGPUModelRunnerOutput(
                ...
                eldr_signatures=pending_eldr_signatures,
            )
        with record_function_or_nullcontext(
