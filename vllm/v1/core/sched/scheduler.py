...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Keep polling outbound KV transfers until their blocks can be freed.
        # Backport of vllm-project/vllm#41180.
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if model_runner_output.eldr_signatures is not None:
                request.eldr_signature = model_runner_output.eldr_signatures.get(req_id)
            generated_token_ids = (
    ...
    def _update_request_with_output(
        ...
        for num_new, output_token_id in enumerate(new_token_ids, 1):
            ...
            transfer = request.kv_transfer_params
            if not stopped and transfer and transfer.get("prefill_first"):
                # Preserve the original sampling budget/min_tokens on P. Only
                # the engine execution stops after one sampled output token.
                transfer["prefill_continues"] = True
    ...
    def _free_request(
        ...
        if not delay_free_blocks:
            ...
            # The connector still owns these blocks during the transfer.
    ...
    def has_delayed_free_requests(self) -> bool:
        """Whether an outbound KV transfer still holds request blocks."""
        return len(self.delayed_free_req_ids) > 0

    def has_requests(self) -> bool:
        """Include outbound transfers so their acknowledgements are drained."""
        return (
