...
class Request:
    # Only the opt-in output transport writes an instance value. Ownership follows
    # the request, including cancellation; RR allocates no signature state.
    eldr_signature: bytes | None = None
    ...
    def __init__(
        ...
        if not abort_immediately:
            from vllm.eldr.prefill_first import first_token_ids

            if tokens := first_token_ids(sampling_params):
                self.append_output_token_ids(tokens)
