...
from vllm.eldr import capture as eldr_capture
...
class MoERunner(MoERunnerInterface):
    ...
    def _apply_quant_method(
        ...
        if self._quant_method.is_monolithic:
            callback = self.router.capture_fn
            exact = callback is not None and eldr_capture.supports(
                self._quant_method, layer
            )
            if callback is not None and not exact:
                raise ValueError(
                    "ELDR capture requires a backend exposing its actual expert IDs"
                )
            context = (
                eldr_capture.observe(callback)
                if exact
                else nullcontext()
            )
            with context:
